...
try:
    dataset = pd.read_csv(dataset_path + 'chapter2_own.csv', index_col=0)
except IOError as e:
    print('File not found, try to run previous crowdsignals scripts first!')
    raise e

dataset.index = dataset.index.to_datetime()
dataset = dataset.rename(columns={'acc_label': 'label'})
...
dataset.to_csv(dataset_path + 'chapter3_own_outliers.csv')
