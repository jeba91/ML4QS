...
dataset = pd.read_csv(dataset_path + 'chapter3_own_outliers.csv', index_col=0)
...
# DataViz.plot_dataset(new_dataset.ix[int(0.4*len(new_dataset.index)):int(0.43*len(new_dataset.index)), :], ['acc_X (m/s^2)', 'acc_X (m/s^2)_lowpass'], ['exact','exact'], ['line', 'line'])
...
periodic_measurements = ['acc_X (m/s^2)', 'acc_Y (m/s^2)', 'acc_Z (m/s^2)', 'gyr_X (rad/s)', 'gyr_Y (rad/s)', 'gyr_Z (rad/s)', 'mag_X (muT)', 'mag_Y (muT)', 'mag_Z (muT)']
...
dataset.to_csv(dataset_path + 'chapter3_own_final.csv')
