...
dataset = pd.read_csv(dataset_path + 'chapter3_final_outliers.csv', index_col=0)
...
# kalman_dataset = KalFilter.apply_kalman_filter(original_dataset, 'acc_Acceleration x (m/s^2)')
# DataViz.plot_imputed_values(kalman_dataset, ['original', 'kalman'], 'acc_Acceleration x (m/s^2)', kalman_dataset['acc_Acceleration x (m/s^2)_kalman'])
# DataViz.plot_dataset(kalman_dataset, ['acc_Acceleration x (m/s^2)', 'acc_Acceleration x (m/s^2)_kalman'], ['exact','exact'], ['line', 'line'])
...
# new_dataset = LowPass.low_pass_filter(copy.deepcopy(dataset), 'acc_Acceleration x (m/s^2)', fs, cutoff, order=10)
# DataViz.plot_dataset(new_dataset.ix[int(0.4*len(new_dataset.index)):int(0.43*len(new_dataset.index)), :], ['acc_Acceleration x (m/s^2)', 'acc_Acceleration x (m/s^2)_lowpass'], ['exact','exact'], ['line', 'line'])
#
