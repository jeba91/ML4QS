...
try:
    dataset = pd.read_csv(dataset_path + 'chapter4_time.csv', index_col=0)
except IOError as e:
...
periodic_predictor_cols = ['acc_Acceleration x (m/s^2)', 'acc_Acceleration y (m/s^2)', 'acc_Acceleration z (m/s^2)',
  ...
  'linacc_Linear Acceleration x (m/s^2)', 'linacc_Linear Acceleration y (m/s^2)', 'linacc_Linear Acceleration z (m/s^2)', 'hr_Heart Rate']

data_table = FreqAbs.abstract_frequency(copy.deepcopy(dataset), ['linacc_Linear Acceleration x (m/s^2)'], int(float(10000)/milliseconds_per_instance), fs)

# Spectral analysis.

DataViz.plot_dataset(data_table, ['linacc_Linear Acceleration x (m/s^2)_max_freq', 'linacc_Linear Acceleration x (m/s^2)_freq_weighted', 'linacc_Linear Acceleration x (m/s^2)_pse', 'label'], ['like', 'like', 'like', 'like'], ['line', 'line', 'line','points'])
...
dataset.to_csv(dataset_path + 'chapter4_freq.csv')
