...
try:
    dataset = pd.read_csv(dataset_path + 'chapter8_result.csv', index_col=0)
except IOError as e:
...
basic_features = ['acc_phone_x','acc_phone_y','acc_phone_z','acc_watch_x','acc_watch_y','acc_watch_z','gyr_phone_x','gyr_phone_y','gyr_phone_z','gyr_watch_x','gyr_watch_y','gyr_watch_z',
                  ...
                  'acc_phone_total', 'acc_watch_total','gyr_phone_total','gyr_watch_total','mag_phone_total','mag_watch_total']
...
['temp_pattern_labelOnTable','labelOnTable','temp_pattern_labelOnTable(b)labelOnTable',
...
for i in range(0, len(possible_feature_sets)):
    selected_train_X = train_X[possible_feature_sets[4]]
    selected_test_X = test_X[possible_feature_sets[4]]
    print(selected_train_X.columns)
    ...
    columns = [hah
    'self.hr_watch_rate']
    equations = ['self.pca_1_temp_mean_ws_120 + self.pca_2_temp_mean_ws_120',
    'self.a * self.pca_1_temp_mean_ws_120',
    'self.b * self.pca_2_temp_mean_ws_120',
    'self.pca_2 * self.acc_watch_y_temp_mean_ws_120',
    'self.c * self.pca_2',
    'self.d * self.acc_watch_y_temp_mean_ws_120',
    'self.gyr_watch_y_pse + self.gyr_watch_x_pse',
    'self.e * self.gyr_watch_y_pse',
    'self.f * self.gyr_watch_x_pse']
    targets = ['self.hr_watch_rate']
    parameters = ['self.a', 'self.b', 'self.c', 'self.d','self.e','self.f']
    repeats = 10

    for repeat in range(0, repeats):
        print '----', repeat
        result_nsga = learner.dynamical_systems_model_nsga_2(
        selected_train_X,
        train_y.to_frame(name='hr_watch_rate'),
        selected_test_X, test_y.to_frame(name='hr_watch_rate'),
        columns, equations, targets, parameters)
        print(result_nsga[0][1], result_nsga[0][3])
        plot = DataViz.plot_numerical_prediction_versus_real(train_X.index, train_y, result_nsga[0][0]['hr_watch_rate'],test_X.index, test_y, result_nsga[0][2]['hr_watch_rate'], 'heart rate')
        savename = str(i)+str(repeat)+'.pdf'
